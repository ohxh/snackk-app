...
class SpecificProduct extends DisplayableAsProduct with ChangeNotifier {
  ...
  SpecificProduct.fromGeneric(GenericProduct p)
    ...
    p.attributes
        .forEach((a) => attributes.add(SpecificAttribute.fromGeneric(a, this)));
    if (p.sizes().keys.length == 1) this.size = p.sizes().keys.first;
  }
  ...
  int get singlePrice {
    ...
    return this.base.price.evaluate(size) +
        attributes.fold(0, (acc, g) => acc + g.price());
  }
  ...
  dynamic get attributeJson {
    ...
    attributes.forEach(
        (a) => json[a.base.name] = a.selection.map((a) => a.name).toList());
    return json;
  }
  ...
  factory SpecificProduct.fromCached(CachedProduct cached, Menu menu) {
    List<GenericProduct> allProducts = menu.categories
        .fold(<GenericProduct>[], (acc, c) => acc..addAll(c.products));

    SpecificProduct result = SpecificProduct.empty();
    result.quantity = cached.quantity;
    result.base = allProducts
        .singleWhere((p) => p.name.toLowerCase() == cached.name.toLowerCase());
    result.size = result.base
        .sizes()
        .keys
        .singleWhere((a) => a.toLowerCase() == cached.size.toLowerCase());
    result.uuid = Uuid().v1();
    result.attributes = List();

    result.base.attributes.forEach((a) => result.attributes.add(
        SpecificAttribute.fromCached(
            a,
            cached.attributes[cached.attributes.keys
                .singleWhere((x) => x.toLowerCase() == a.name.toLowerCase())],
            result)));
