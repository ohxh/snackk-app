...
class BreveTransaction extends Deserialized {
  ...
  BreveTransaction.fromDocument(DocumentSnapshot doc) : super.fromDocument(doc) {
    source = doc["source"];
    destination = doc["destination"];
    destinationString = doc["destinationString"];
    sourceString = doc["sourceString"];
    timestamp = doc["timestamp"] != null ? DateTime.fromMicrosecondsSinceEpoch(doc["timestamp"]) : null;
    amount = doc["amount"];
    type = doc["type"];
  }
  ...
  String get titleString {
    ...
    case "reload": return "Reload from " + sourceString;
    case "wallet-payment": return "Wallet payment to " + destinationString;
    case "wallet-reload": return "Reload from " + sourceString;
